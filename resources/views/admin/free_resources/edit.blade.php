...
<div  id="app">
    <main class="">
       ...
       <div class="container">
        ...
        <div class="app-title row">
            <div class="col-md-10">
                <h1><i class="fa fa-plus-circle"></i> Edit Free Resource</h1>
                
            </div>
            
            <div class="col-md-2">
            <a class="btn btn-primary" href="{{url('admin/free-resources')}}"><i class="fa fa-list"></i> Free Resources</a>
            </div>
        </div>
        </div>
            ...
            <div class="alert alert-success msgalt" id="successMsg" style="display:none;">
             <a title="close" aria-label="close" data-dismiss="alert" class="close" href="#">×</a><span id='insSuccess'>Free Resource updated Successfully</span>
           </div>
        ...
        <div class="panel panel-light h-auto">
                                    ...
                                    <div class="panel-header">
                                       {{--  <h1 class="panel-title">Edit Free Resource</h1> --}}
                                       
                                    </div>
                             
                                    
                                    <div class="panel-body">
                                       <form name="" id="resourceReg" enctype="multipart/form-data" method="POST"action="{{URL('free-resources/edit/'.$free_resource->id)}}" class="form-horizontal" @submit.prevent="onSubmit">
        ...
        <li class="nav-item">
            <a class="nav-link" id="custom-tab-2" data-toggle="tab" href="#custom-tab-content-2" role="tab" aria-controls="custom-tab-content-2" aria-selected="false">Arabic</a>
                        ...
                        <input type="hidden" id="def_lan" name="def_lan" value="en">
                          <input type="hidden" id="cmpId" name="cmpId" value="{{ $free_resource->id }}">
                            <div class="col-lg-12" >
                                {{csrf_field()}}
  ...
  <div class="['form-group', errors.title_en ? 'has-error' : '']" style="margin-bottom:15px;">
                                    ...
                                    <input id="title_en" name="title_en" value="{!! $free_resource->title_en !!}"  autofocus="autofocus" class="form-control" type="text" v-model="form.title_en">
                            ...
                            <strong id="company_name_enerror" style="color: #d00505;"></strong>
                                ...
                                <div class="form-group @if($errors->first('short_desc_en')) has-danger @endif not-external">
                                    ...
                                    <textarea class="form-control @if($errors->first('short_desc_en')) is-invalid @endif" name="short_desc_en" id="short_desc_en" rows="3">{!! $free_resource->short_desc_en !!}</textarea>
          ...
          <div class="form-group @if($errors->first('main_desc_en')) has-danger @endif not-external">
                                    ...
                                    <textarea class="form-control @if($errors->first('main_desc_en')) is-invalid @endif" name="main_desc_en" id="main_desc_en" rows="3">{!! $free_resource->description_en !!}</textarea>
  ...
  <div :class="['form-group', errors.title_ar ? 'has-error' : '']" style="margin-bottom:15px;">
                                    ...
                                    <input id="title_ar" name="title_ar" value="{!! $free_resource->title_ar !!}"  autofocus="autofocus" class="form-control" type="text" v-model="form.title_ar">
                                    <span class="text-danger">
                            <strong id="title_arerror" style="color: #d00505;"></strong>
                                ...
                                <div class="form-group @if($errors->first('short_desc_ar')) has-danger @endif not-external">
                                    ...
                                    <textarea class="form-control @if($errors->first('short_desc_ar')) is-invalid @endif" name="short_desc_ar" id="short_desc_ar" rows="3">{!! $free_resource->short_desc_ar !!}</textarea>
          ...
          <div class="form-group @if($errors->first('main_desc_ar')) has-danger @endif not-external">
                                    ...
                                    <textarea class="form-control @if($errors->first('main_desc_ar')) is-invalid @endif" name="main_desc_ar" id="main_desc_ar" rows="3">{!! $free_resource->description_ar !!}</textarea>
                                        ...
                                        <optgroup label="{{$category->category_name}}">
                                            @if($category->sub_categories->count())
                                                @foreach($category->sub_categories as $sub_category)
                                                    <option value="{{$sub_category->id}}" {{selected($free_resource->second_category_id, $sub_category->id )}} >{{$sub_category->category_name}}</option>
                                                @endforeach
                                            @endif
                                        </optgroup>
                                    @endforeach
 ...
 <div class="row">
          ...
          <div class="form-group @if($errors->first('name')) has-error @endif">
                ...
                <div class="col-sm-10 choose-img-div">
                                        <img class="profile_pic" name="profile_pic" id="profile_pic"  width="150" height="auto" src="@if($free_resource->img) {!! asset('uploads/free_resources_images/'.$free_resource->img) !!} @else {!! asset('admin/img/administrator.png') !!} @endif">
                                       <input type="file" name="profile_file" id="profile_file" class="profile_file" accept="image/*" capture style="display:none">
                                       <img src="{!! asset('admin/images/file_choose.png') !!}" id="upfile1" class="choose-img" style="cursor:pointer" title="Change Profile Picture" />
                                    </div>
          ...
          <div class="form-group @if($errors->first('pre_language')) has-error @endif">
            ...
            @foreach($all_languages as $language)
             <option @if(in_array($language->id, $selLanguages)) selected="" @endif value="{{ $language->id }}">{{ $language->en_language }}</option>
            @endforeach
            </select>
       ...
       <div class="col-md-6 mb15">
          <div class="form-group @if($errors->first('skills')) has-error @endif">
            ...
            <option @if(in_array($skill->id, $selSkills)) selected="" @endif value="{{ $skill->id }}">{{ $skill->en_skill }}</option>
            @endforeach
            </select>
          ...
          <div class="col-xs-12">
            <div class="form-group">
               ...
               <input type="file" name="document" class="form-control" value="">  @if($free_resource->document) <a href="@if($free_resource->document){!! asset('uploads/free_resources_documents/'.$free_resource->document) !!} @endif" target="_blank" class="default-btn"><i class='fa fa-download icon-arrow before'></i><span class="label">View File</span></a> @endif
            </div>
          </div>
...
$(document).ready(function(){
 ...
 $('#resourceReg').on('submit', function(event){
  ...
  $.ajax({
   url:"{{URL('admin/free-resources/edit')}}"+'/'+ cmp_id,
   method:"POST",
